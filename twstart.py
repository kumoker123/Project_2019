...
browser = 'C:/chromedriver_win32/chromedriver.exe'
RESULT_PATH = './'
...
def crawler(query, startdate, enddate):
    ...
    while not enddate == startdate:
        ...
        url = 'https://twitter.com/search?q=' + query + '%20since%3A' + str(startdate) + '%20until%3A' + str(
    ...
    df.to_excel('twitter_result.xlsx', sheet_name='sheet1')
...
def main():
    info_main = input(
        "=" * 50 + "\n" + "트위터 크롤러" + "\n" + "입력 형식에 맞게 입력해주세요." + "\n" + " 시작하시려면 Enter를 눌러주세요." + "\n" + "=" * 50)
    query = input("검색어 입력: ")
    startdate = input("검색 시작 날짜 입력(yyyy-mm-dd): ")
    enddate = input("검색 끝 날짜 입력(yyyy-mm-dd): ")
